<template>
  <div id="addBackNotice">
    <el-page-header @back="router.go(-1)">
      <template #content>
        <span class="topTitle">添加后台公告</span>
      </template>
    </el-page-header>
    <div class="infoContainer">
      <div ref="fromContainerRef" class="fromContainer">
        ...
        <div class="btnItem">
          <el-button @click="pubNotice" type="primary">{{ routerPath == 'updateBackNotice' ? '修改' : '发布'
          }}公告</el-button>
        </div>
      </div>
...
<script lang='ts'>
import { defineComponent, reactive, ref, getCurrentInstance, ComponentInternalInstance, toRefs, onMounted, shallowRef, onBeforeUnmount, watch } from 'vue'
import { useRouter } from "vue-router";
import type { FormInstance } from "element-plus";
import '@wangeditor/editor/dist/css/style.css' // 引入 css
import { Editor, Toolbar } from '@wangeditor/editor-for-vue'
import { InitData } from "@/types/noticeView/backNoticeInfo";
import noticeContent from "@/components/noticeContent.vue";
import glovar from "@/static/globalVar";
import { addNotice, updateNotice, getNoticeInfo } from "@/network/backNoticeInfo";
      ...
      noticeInfoFormRef.value.validate((vaild: boolean) => {
        if (vaild) {
          if (data.noticeForm.valueHtml == "<p><br></p>") {
            ...
            if (data.routerPath == 'updateBackNotice') {
    ...
    onMounted(() => {
      ...
      if (data.routerPath == "updateBackNotice") {
        getNoticeInfo({
          ...
          data.noticeForm.pub_name = res.data.data.name
